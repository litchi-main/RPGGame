...
class RPGState:
    ...
    def get_possible_actions(self):
        """Return a list of possible actions based on current state."""
        # Here we assume possible actions are moving, attacking, or defending
        return ["move", "attack", "defend"]

    def apply_action(self, action):
        """Return the new state after applying an action."""
        # In a real RPG game, we would modify the state based on the action
        new_state = RPGState(f"new_state_after_{action}")
        return new_state

    def is_goal_state(self):
        """Check if this state is the goal state."""
        # Let's assume the goal state is when the state_info is 'goal'
        return self.state_info == "goal"


class RPGGame:
    """A simple RPG game where the player fights against an AI."""

    def __init__(self):
        self.player = Character("Player", 100, 20, 15)
        self.ai = Character("AI", 100, 20, 15)
        self.turn = "player"

    def play(self):
        """Main game loop."""
        while self.player.health > 0 and self.ai.health > 0:
            print(f"Player HP: {self.player.health}, AI HP: {self.ai.health}")
            print(f"Player shield uses left: {self.player.shield_uses}, AI shield uses left: {self.ai.shield_uses}")
            print(f"Player Heal left: {self.player.heal_uses}, AI Heal left: {self.ai.heal_uses}")

            if self.turn == "player":
                self.player_turn()
                self.turn = "ai"
            else:
                self.ai_turn_with_dfs()  # AI now uses DFS for decision-making
                self.turn = "player"
    ...
    def is_goal_state(self):
        """Check if the current game state is a goal state."""
        return self.player.health <= 0 or self.ai.health <= 0

    def get_possible_actions(self):
        """Return the possible actions for the current player (player or AI)."""
        actions = []
        if self.turn == "player":
            if self.player.heal_uses > 0:
                actions.append("heal")
            if self.player.shield_uses > 0:
                actions.append("shield")
            actions.append("attack")  # Player can always attack
        else:  # AI's turn
            if self.ai.heal_uses > 0:
                actions.append("heal")
            if self.ai.shield_uses > 0:
                actions.append("shield")
            actions.append("attack")  # AI can always attack
        return actions
    ...
    def player_turn(self):
        ...
        if choice == "1":
            ...
            if self.player.shield_uses > 0:
                ...
                print("You used your shield!")
            else:
                print("No shield uses left!")
        elif choice == "3":
            if self.player.heal_uses > 0:
                self.player.heal()
    ...
    def ai_turn_with_dfs(self):
        """Handles the AI's turn using DFS for decision-making."""
        print("\nAI's turn:")

        # Create the problem with the current game state (self)
        problem = RPGProblem(self)  # Only pass the game state
        result_node = depth_first_graph_search(problem)

        if result_node:
            # If DFS finds a valid result, we execute the best action
            possible_actions = result_node.state.get_possible_actions()  # Get possible actions for current state
            action = dfs_decision_making(
                possible_actions)  # You can implement the DFS logic here or use an existing method

            print(f"AI chose to {action}")

            # Now apply the chosen action
            new_game = result_node.state.apply_action(action)

            if action == "attack":
                damage = self.ai.attack()
                ...
                if self.ai.shield_uses > 0:
                    ...
                    print("AI has no shield uses left!")
            elif action == "heal":
                if self.ai.heal_uses > 0:
                    self.ai.heal()
                else:
                    print("AI has no heal uses left!")
        else:
            print("AI failed to make a decision!")

    def apply_action(self, action):
        """Apply the action to the current game state and return the resulting state."""
        new_game = RPGGame()  # Create a new game instance to represent the new state
        new_game.player = self.player.copy()  # Assuming you have a `copy` method to clone the player
        new_game.ai = self.ai.copy()  # Same for the AI
        new_game.turn = self.turn

        if action == "attack":
            if self.turn == "player":
                damage = new_game.player.attack()
                new_game.ai.take_damage(damage)
            else:  # AI's turn
                damage = new_game.ai.attack()
                new_game.player.take_damage(damage)
        elif action == "shield":
            if self.turn == "player":
                if new_game.player.shield_uses > 0:
                    new_game.player.use_shield()
                else:
                    print("No shield uses left!")
            else:  # AI's turn
                if new_game.ai.shield_uses > 0:
                    new_game.ai.use_shield()
                else:
                    print("AI has no shield uses left!")
        elif action == "heal":
            if self.turn == "player":
                if new_game.player.heal_uses > 0:
                    new_game.player.heal()
                else:
                    print("No heal uses left!")
            else:  # AI's turn
                if new_game.ai.heal_uses > 0:
                    new_game.ai.heal()
                else:
                    print("AI has no heal uses left!")

        return new_game  # Return the new game state after applying the action
...
class Character:
    ...
    def copy(self):
        ...
        new_character = Character(self.name, self.health, self.attack_power, self.defense)
        new_character.shield_uses = self.shield_uses
        new_character.heal_uses = self.heal_uses
        return new_character

    def attack(self):
        """Calculates and returns the damage dealt by the character's attack."""
    ...
    def heal(self):
        """Heals the character."""
        if self.heal_uses > 0:
            self.health += 20  # Healing amount (e.g., 20)
            self.heal_uses -= 1

    def use_shield(self):
        """Activates the shield to reduce incoming damage."""
        if self.shield_uses > 0:
            self.shield_uses -= 1
            self.defense += 10  # Example: increase defense by 10 points

class NuRPG(Game):
    """Yra du veikėjai: P1 ir P2 (P1 pradeda).
    Būsena turi veikėją, kuris atlieka vieksmą, išsaugota naudingumą (utility),
    galimų veiksmų sąrašą, kurį sudaro ataka (nuema priešo gyvybes), gynyba
    (sumazina ateities priešo atakas, tačiau turi paaukoti gyvybes) ir vienkartinis,
    globalus gydymas (vienam veikėjui jį panaudojus, kitas nebegali pasigydyti),
    "lenta", kurioje saugomi mūsų veikėjų duomenys (gyvybės, atakos reikšmė, gynybos reikšmė)"""

    def __init__(self, h=100, a=20, d=15):
        #h - gyvybes, a - atakos reikšmė, d - gynybos reikšmė
        charPreset = namedtuple('character', 'health, attack, defense')
        player1 = charPreset(h, a, d)
        player2 = charPreset(h, a, d)
        board = {}
        board["P1"] = player1
        board["P2"] = player2
        moves = ["a", "d", "h"]
    ...
    def actions(self, state):
        #kokios atakos yra galimos bet kokiam veikėjui
        return state.moves

    def result(self, state, move):
        #apskaičiuoja kiekvienos atakos rezultatą
        if move not in state.moves:
            return state  # Illegal move has no effect
        board = state.board.copy()
        #kiekvieną ėjima gynybos reikšmės yra mažinamos, yra poreikis jas didinti
        board["P1"] = board["P1"]._replace(defense = board["P1"].defense - 5 if board["P1"].defense > 0 else 0)
        board["P2"] = board["P2"]._replace(defense = board["P2"].defense - 5 if board["P2"].defense > 0 else 0)
        moves = list(state.moves)
        if move == "a":
            if state.to_move == 'X':
                board["P2"] = board["P2"]._replace(health=board["P2"].health - board["P1"].attack * (100 - board["P2"].defense) / 100)
            else:
                board["P1"] = board["P1"]._replace(health=board["P1"].health - board["P2"].attack * (100 - board["P1"].defense) / 100)
        if move == "d":
            if state.to_move == 'X':
                board["P1"] = board["P1"]._replace(defense=board["P1"].defense + 10)
                board["P1"] = board["P1"]._replace(health=board["P1"].health - board["P1"].defense)
            else:
                board["P2"] = board["P2"]._replace(defense=board["P2"].defense + 10)
                board["P2"] = board["P2"]._replace(health=board["P2"].health - board["P2"].defense)

        if move == "h":
            if state.to_move == 'X':
                    board["P1"] = board["P1"]._replace(health=board["P1"].health + 20)
                    moves.remove("h")
            else:
                    board["P2"] = board["P2"]._replace(health=board["P2"].health + 20)
                    moves.remove("h")
    ...
    def display(self, state):
        #žaidimo gale pavaizduoja, kas laimi ir su kiek gyvybių liko
        board = state.board
        if self.compute_utility(board, 'X') == 1:
            print("P1 won")
            print(str(board["P1"].health) + "hp left")
        else:
            print("P2 won")
            print(str(board["P2"].health) + "hp left")
